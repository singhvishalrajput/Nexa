...
- Home, Accounts, Payments, Cards and Transactions are the primary destinations. Native “More banking” disclosure retains Overview, Send money, Payees, Bills, Direct debits, Scheduled payments, Loans, and role-gated banking operations. It opens automatically for the active secondary route.
...
Verification: 65 tests pass, lint reports zero errors across 35 source files, type-check and optimized release build pass. Navigation regression tests cover complete route reachability without duplicate destinations, administrator visibility, current-page semantics, secondary disclosure and Hindi labels. Existing draft, payment, authentication and recovery tests still pass.
