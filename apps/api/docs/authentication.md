...
Successful startup includes Flyway reporting schema version 11 and Tomcat listening on port 8088.

## 1. Verify public and protected routes

`GET http://localhost:8088/api/v1/health` must return `200 OK` without a token.

`GET http://localhost:8088/api/v1/me` must return `401 Unauthorized` without a token.
...
GET http://localhost:8088/api/v1/accounts
GET http://localhost:8088/api/v1/accounts/{accountId}/balance
GET http://localhost:8088/api/v1/accounts/{accountId}/transactions
```
