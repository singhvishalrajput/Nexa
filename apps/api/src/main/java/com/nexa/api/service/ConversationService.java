...
public class ConversationService {
  ...
  private ConversationInterpreter.Interpretation interpretWithContext(String id, String text) {
    ...
    var simpleRead = com.nexa.api.beans.FastBankingIntent.match(text);
    if (simpleRead == com.nexa.api.beans.Intent.GET_BALANCE
        || simpleRead == com.nexa.api.beans.Intent.GET_RECENT_TRANSACTIONS) {
      var latest =
          db.query(
    ...
    var recent =
        db.query(
            ...
            this::turn,
            id);
    var messages = new java.util.ArrayList<com.nexa.api.service.OllamaInterpreter.Message>();
    for (int i = recent.size() - 1; i >= 0; i--) {
      var turn = recent.get(i);
      if ("PRIVACY".equals(turn.intent())) continue;
      messages.add(new com.nexa.api.service.OllamaInterpreter.Message("user", turn.userText()));
      // No account/card payloads, credentials or database snapshots are sent to the model.
      messages.add(
          new com.nexa.api.service.OllamaInterpreter.Message("assistant", turn.assistantText()));
    }
