...
const navigation: Array<{
    ...
    { label: "Overview", page: "overview", icon: "◫", group: "WORKSPACE" }, { label: "Accounts", page: "accounts", icon: "▤" }, { label: "Transactions", page: "transactions", icon: "⇄" }, { label: "Payments", page: "payments", icon: "↗" }, { label: "Payees", page: "beneficiaries", icon: "♧" },
...
export function BankingApp() {
    ...
    useEffect(() => { restore(); const change = () => setRoute(parseRoute(window.location.hash)); const expired = () => { generation.current++; setSession(null); setNotice("Your session has expired. Sign in again to continue."); go("login"); }; window.addEventListener("hashchange", change); window.addEventListener("nexa-session-expired", expired); return () => { generation.current++; window.removeEventListener("hashchange", change); window.removeEventListener("nexa-session-expired", expired); }; }, []);
    async function signOut() { generation.current++; setSession(null); go("login"); try {
        await logout();
...
function Workspace({ session, setSession, signOut, route }: {
    ...
    const page = route.page === "login" || route.page === "register" ? "overview" : route.page;
    useEffect(() => { setMenu(false); window.scrollTo(0, 0); document.title = (page === "overview" ? "Overview" : navigation.find(n => n.page === page)?.label || "Nexa") + " · Nexa"; heading.current?.querySelector<HTMLElement>("h1")?.focus(); }, [page, route.id, data.loading]);
    const endSession = async () => { if (signingOut)
        return; setSigningOut(true); await signOut(); };
    if (page === "assistant")
        return <ConversationWorkspace session={session} onClose={() => go("overview")} onLogout={endSession}/>;
    const accountPage = ["overview", "accounts", "transactions", "payments", "settings"].includes(page);
    function content() {
