...
export function AdminApp({ session, signOut, route }: {
    ...
    return <div class="bank-app admin-app experience-admin"><WorkspaceRail page="admin" name={session.profile.fullName} admin/><div class="experience-admin-body"><header class="bank-topbar"><a class="bank-wordmark" href="#home" aria-label="Nexa home">nexa · Administration</a><div><span>{session.user.email}</span><button class="bank-button secondary" disabled={signingOut} onClick={exit}>Sign out</button></div></header><main class="bank-main">
 <nav class="admin-sections" aria-label="Administration"><a href="#/admin" aria-current={route.section!=="loans"?"page":undefined}>Accounts</a><a href="#/admin/loans" aria-current={route.section==="loans"?"page":undefined}>Loan requests{loans.data?.length?` (${loans.data.length})`:""}</a></nav>
 ...
 <PageHeading eyebrow="ADMINISTRATOR WORKSPACE" title="Find an account" description="Search by account number, customer name or email. Open an account to see everything connected to it." action={<button class="bank-button secondary" onClick={data.reload}>Refresh accounts</button>}/>
...
function AccountTable({ accounts }: {
    accounts: Account[];
}) { return <div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Account</th><th>Customer</th><th>Type</th><th>Status</th><th>Balance / outstanding</th></tr></thead><tbody>{accounts.map(a => <tr key={a.id}><td><a class="admin-account-link" href={accountHref(a.id)}>{a.name} <span aria-hidden="true">→</span></a><small>{a.number}</small></td><td>{a.customerName || "System"}<small>{a.customerEmail}</small></td><td>{a.type}</td><td><Status value={a.status}/></td><td>{formatMoney(a.balance, a.currency)}</td></tr>)}</tbody></table></div>; }
...
export function AccountEditor({ token, account, close, done }: {
    ...
    return <Modal title={account.name} onClose={close} locked={busy}><div class="bank-form"><dl><Detail label="Account number">{account.number}</Detail><Detail label="Customer">{account.customerName || "System"}</Detail><Detail label="Current balance / outstanding">{formatMoney(account.balance, account.currency)}</Detail></dl>
 <fieldset disabled={busy || uncertain} class="bank-transfer-fields"><label>Account name<input required value={name} maxLength={120} onInput={e => setName(e.currentTarget.value)}/></label><label>Status<select value={status} onChange={e => setStatus(e.currentTarget.value)}>{["ACTIVE", "BLOCKED", "CLOSED"].map(s => <option value={s}>{s}</option>)}</select></label><label>Reason for change<textarea required maxLength={500} value={reason} onInput={e => { setReason(e.currentTarget.value); setReview(false); }}/></label><button class="bank-button" disabled={!name.trim() || !reason.trim()} onClick={() => save(false)}>Save account details</button>
 {account.category === "CUSTOMER" && ["SAVINGS", "CURRENT"].includes(account.type) && <><h3>Post a balance adjustment</h3><p>Credits and debits create a transaction and balanced accounting entries. The account must be active.</p><label>Adjustment<select value={direction} onChange={e => { setDirection(e.currentTarget.value); setReview(false); }}><option value="CREDIT">Credit / deposit</option><option value="DEBIT">Debit / withdrawal</option></select></label><label>Amount ({account.currency})<input inputMode="decimal" value={amount} onInput={e => { setAmount(e.currentTarget.value); setReview(false); }}/></label><button class="bank-button secondary" disabled={!validAmount(amount) || !reason.trim()} onClick={() => setReview(true)}>Review adjustment</button></>}
 </fieldset>
