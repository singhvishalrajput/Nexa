...
export function PaymentsPage({ token, accounts }: {
    ...
    async function prepare(e: Event) { e.preventDefault(); if (lock.current)
    ...
    return <><PageHeading title="Payments" description="Review payment details and check eligibility before your next step."/><div class="bank-notice"><span aria-hidden="true">ⓘ</span><p><strong>Online payment execution isn’t available yet.</strong> You can validate details with your bank. No money will move and no mandate will be cancelled.</p></div><div class="bank-detail-grid"><Panel title="Review a payment"><form class="bank-form" onSubmit={prepare}><label>What would you like to review?<select value={operation} disabled={busy} onChange={e => { setOperation(e.currentTarget.value); setTargetPage(0); setTarget(""); setResult(undefined); }}><option value="START_TRANSFER">Transfer to a payee</option><option value="PAY_BILL">Bill payment</option><option value="PAY_CARD">Credit card payment</option><option value="CANCEL_MANDATE">Cancel a direct debit</option></select></label><label>From account<select required disabled={busy} value={account} onChange={e => setAccount(e.currentTarget.value)}><option value="">Select an account</option>{accounts.filter(a => a.status === "ACTIVE").map(a => <option value={a.id}>{a.displayName} · {a.accountNumberMasked}</option>)}</select></label><State loading={targets.loading} error={targets.error} retry={targets.reload}><label>{operation === "START_TRANSFER" ? "Payee" : "Banking record"}<select required disabled={busy} value={target} onChange={e => setTarget(e.currentTarget.value)}><option value="">Select a record</option>{targets.data?.map(p => <option value={p.id}>{productTitle(p)} · {p.status.toLowerCase()}</option>)}</select></label>{!targets.data?.length && <p>No eligible records are available. Your bank must supply them first.</p>}{kind !== "beneficiaries" && <div class="bank-pagination"><span>Records page {targetPage + 1}</span><button type="button" disabled={busy || targetPage === 0} onClick={() => { setTarget(""); setTargetPage(p => p - 1); }}>Previous records</button><button type="button" disabled={busy || (targets.data?.length || 0) < 12} onClick={() => { setTarget(""); setTargetPage(p => p + 1); }}>Next records</button></div>}</State>{operation !== "CANCEL_MANDATE" && <label>Amount (INR)<input required inputMode="decimal" pattern="(?:0|[1-9][0-9]{0,12})(?:\.[0-9]{1,2})?" placeholder="0.00" value={amount} disabled={busy} onInput={e => setAmount(e.currentTarget.value)}/><small>Enter a positive amount with at most two decimal places.</small></label>}{error && <p role="alert" class="bank-error">{error}</p>}<button class="bank-button" disabled={busy || !account || !target || (operation !== "CANCEL_MANDATE" && !validAmount(amount))}>{busy ? "Checking details…" : "Check payment details"}</button></form></Panel><Panel title="Before you continue"><div class="bank-form"><span class="bank-tile-symbol">↗</span><h3>Clear details. No surprises.</h3><p>Check the source account, recipient and amount carefully. A successful review checks eligibility only.</p><p>To complete a payment or change a mandate, use your bank’s supported payment channel.</p><a href="#/beneficiaries">View saved payees ↗</a><a href="#/scheduled-payments">View scheduled payments ↗</a></div></Panel></div>{result && <Modal title="Details checked — not submitted" onClose={() => setResult(undefined)}><div class="bank-form"><Status value={result.status}/><dl><Detail label="From account">{accounts.find(a => a.id === result.accountId)?.accountNumberMasked}</Detail><Detail label="Recipient / record">{targets.data?.find(p => p.id === result.targetId) && productTitle(targets.data.find(p => p.id === result.targetId)!)}</Detail>{result.amount && <Detail label="Amount">{formatMoney(result.amount, result.currencyCode)}</Detail>}</dl><p>Your bank accepted these details for review. {result.executionAvailable ? "This interface does not provide a submission endpoint." : "Execution is not available."} No money has moved and no changes were made.</p><button class="bank-button" onClick={() => setResult(undefined)}>Done</button></div></Modal>}</>;
}
