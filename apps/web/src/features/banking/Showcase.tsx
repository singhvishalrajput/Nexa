...
export function DemoHistory({token}: {token: string}) {
  ...
  return <Panel className="bank-request-history" title="Request history" action={<button class="bank-button secondary" type="button" onClick={data.reload}>Refresh</button>}><State loading={data.loading} error={data.error} retry={data.reload} empty={!data.loading && !data.data?.length ? "No requests yet." : undefined}>{data.data?.map(r => <div class="bank-record bank-clickable-card" key={r.id}><div><strong>{humanize(r.operation)}</strong><small>{requestStatus(r.status)} · {referenceLabel(r.reference || r.id)}</small></div><button class="bank-button secondary bank-card-primary" type="button" aria-label={"View request: " + humanize(r.operation)} onClick={() => setSelected(r)}>View request</button></div>)}</State>{selected && <DemoConfirmation key={selected.id} token={token} receipt={selected} onClose={() => {setSelected(undefined); data.reload();}}/>}</Panel>;
}
