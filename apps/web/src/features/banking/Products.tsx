...
import { CardSupport, DigitalDebitCreate, ProductPayment } from "./CardActions";
...
export function ProductsPage({ token, kind, id, initiallyOpen = false }: {
 ...
 <State loading={data.loading} error={data.error} retry={data.reload} empty={!data.loading && !data.error && !id && !visible.length ? "No " + productNames[kind].toLowerCase() + " to show" : undefined}>
  {detail ? <ProductDetail key={detail.id} token={token} product={detail} kind={kind} reload={data.reload} onPosted={setReceipt}/> : <div class={(kind === "cards" ? "bank-card-grid" : "bank-product-grid") + " bank-product-grid--" + kind}>
    {visible.map(p => <section key={p.id} class={"bank-product bank-clickable-card " + (kind === "cards" ? "bank-payment-card" : "")}>
      {kind === "cards" && <DigitalCard product={p}/>}
      ...
      <h2><a class="bank-card-primary" href={"#/" + kind + "/" + encodeURIComponent(p.id)}>{productTitle(p)}</a></h2>
      <p>{(p.numberMasked || p.accountNumberMasked || p.customerNumberMasked) ? <SensitiveNumber kind={kind} id={p.id} masked={p.numberMasked || p.accountNumberMasked || p.customerNumberMasked}/> : p.reference || p.bankName || p.category || humanize(kind)}</p>
 ...
 {kind === "cards" && <CardSupport token={token}/>}
 {kind === "cards" && !id && page === 0 && !filter && !search && !data.loading && !data.error && !rows.length && <DigitalDebitCreate token={token} onCreated={data.reload}/>}
...
function ProductDetail({ product: p, kind, token, reload, onPosted }: {
    ...
    return <div class="bank-detail-grid"><Panel className="bank-product-summary bank-clickable-card">{kind === "cards" && <DigitalCard product={p}/>}<span class="bank-tile-symbol">◇</span><h2><a class="bank-card-primary" href={"#/" + kind + "/" + encodeURIComponent(p.id)}>{productTitle(p)}</a></h2><Status value={p.status}/>{amount(p) !== undefined && <>{kind === "loans" && <span class="loan-balance-label">Outstanding principal</span>}<strong>{formatMoney(amount(p)!, p.currencyCode || "INR")}</strong>{kind === "loans" && <small class="loan-balance-note">Borrowed amount still unpaid · excludes future interest</small>}</>}<p>{(p.numberMasked || p.accountNumberMasked || p.customerNumberMasked) ? <SensitiveNumber kind={kind} id={p.id} masked={p.numberMasked || p.accountNumberMasked || p.customerNumberMasked}/> : p.reference}</p><a href={"#/" + kind}>{t("← Back to")} {productNames[kind].toLowerCase()}</a></Panel><Panel title={t("Details")}><dl>{p.bankName && <Detail label={t("Bank")}>{p.bankName}</Detail>}{p.cardType && <Detail label={t("Card type")}>{humanize(p.cardType)}</Detail>}{p.cardType !== "DEBIT" && p.availableLimit !== undefined && <Detail label={t("Available limit")}>{formatMoney(p.availableLimit, p.currencyCode)}</Detail>}{p.cardType !== "DEBIT" && p.creditLimit !== undefined && <Detail label={t("Credit limit")}>{formatMoney(p.creditLimit, p.currencyCode)}</Detail>}{p.cardType !== "DEBIT" && p.minimumPayment !== undefined && <Detail label={t("Minimum payment")}>{formatMoney(p.minimumPayment, p.currencyCode)}</Detail>}{p.dueAt && <Detail label={t("Due date")}>{formatDate(p.dueAt)}</Detail>}{p.nextDebit && <Detail label={t("Next debit")}>{formatDate(p.nextDebit)}</Detail>}{p.frequency && <Detail label={t("Frequency")}>{humanize(p.frequency)}</Detail>}{p.interestRate && <Detail label={t(kind === "loans" ? "Annual interest rate" : "Interest rate")}>{p.interestRate}%</Detail>}{p.nextEmi != null && <Detail label={t(kind === "loans" ? "Next EMI (includes interest)" : "Next instalment")}>{formatMoney(p.nextEmi, p.currencyCode)}</Detail>}{p.accountId && <Detail label={t("Linked account")}><a href={"#/accounts/" + encodeURIComponent(p.accountId)}>{t("View account ↗")}</a></Detail>}{p.reference && <Detail label={t("Reference")}>{p.reference}</Detail>}</dl></Panel>
 {(kind === "mandates" || kind === "loans") && <ProductOperations token={token} kind={kind} product={p} reload={reload} onPosted={onPosted}/>}
 {kind === "cards" && p.cardType === "CREDIT" && p.status === "ACTIVE" && Number(p.outstanding) > 0 && <Panel title="Credit-card payment"><ProductPayment token={token} product={p} operation="PAY_CARD" reload={reload}/></Panel>}
 {kind === "bills" && ["UPCOMING", "DUE", "OVERDUE", "FAILED"].includes(p.status) && <Panel title="Bill payment"><ProductPayment token={token} product={p} operation="PAY_BILL" reload={reload}/></Panel>}
 ...
 {p.paymentHistory && <Panel title={t("Payment history")}><State empty={!p.paymentHistory.length ? "No payments recorded" : undefined}>{p.paymentHistory.map(t => <div class={"bank-record" + (t.reference?.startsWith("TX-") ? " bank-clickable-card" : "")} key={t.id}><div><strong>{t.payee}</strong><small>{formatDate(t.dueAt, true)}</small>{t.reference?.startsWith("TX-") && <a class="bank-card-primary" href={"#/transactions/" + encodeURIComponent(t.reference)}>View transaction · {t.reference}</a>}</div><Status value={t.status}/><strong>{formatMoney(t.amount, t.currencyCode)}</strong></div>)}</State></Panel>}
