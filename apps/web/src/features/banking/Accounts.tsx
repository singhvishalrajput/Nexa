...
export function AccountsPage({ token, id, accounts, reload }: {
    ...
    return <><PageHeading title={id ? "Account details" : t("Your accounts")} description={t("A clear view of every account you hold with Nexa.")} action={!id && <button class="bank-button" onClick={() => setOpening(true)}>{t("＋ Open account")}</button>}/>{id ? <State loading={detail.loading} error={detail.error} retry={detail.reload}>{detail.data && <div class="bank-detail-grid"><AccountTile account={detail.data}/><Panel title={t("Account information")}><dl><Detail label={t("Account name")}>{detail.data.displayName}</Detail><Detail label={t("Account number")}><AccountNumber token={token} id={id!} masked={detail.data.accountNumberMasked}/></Detail><Detail label={t("Status")}><Status value={detail.data.status}/></Detail><Detail label={t("Ledger balance")}>{formatMoney(detail.data.ledgerBalance, detail.data.currencyCode)}</Detail><Detail label={t("Last updated")}>{detail.data.updatedAt ? formatDate(detail.data.updatedAt, true) : "Not available"}</Detail></dl><a class="bank-button" href={"#/transactions?account=" + encodeURIComponent(id)}>{t("View transactions")}</a></Panel></div>}</State> : <div class="bank-account-grid">{accounts.map(a => <AccountTile account={a} key={a.id}/>)}{!accounts.length && <State empty={t("No bank accounts yet")}/>}</div>}{opening && <OpenAccount token={token} close={() => setOpening(false)} done={() => { setOpening(false); reload(); }}/>}</>;
}
