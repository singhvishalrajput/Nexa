...
import { formatMoney, formatDate, completedStatuses, transactionDirection, humanize } from "../../services/banking-content";
...
export function Overview({ token, name, accounts, reload }: {
    ...
    const incoming = recent.data?.content.filter(t => t.amount > 0 && completedStatuses.includes(t.status)) || [];
    const outgoing = recent.data?.content.filter(t => t.amount < 0 && completedStatuses.includes(t.status)) || [];
 ...
 <Panel title={t("Activity at a glance")} className="bank-summary"><State loading={recent.loading} error={recent.error} retry={recent.reload}><p>{t("Completed activity in the latest")} {recent.data?.content.length || 0} {t("transactions for this account.")}</p><div class="bank-summary-number"><span>{t("Money in")}</span><strong class="bank-positive">{formatMoney(sumMoney(incoming.map(t => t.amount)))}</strong></div><div class="bank-summary-number"><span>{t("Money out")}</span><strong>{formatMoney(sumMoney(outgoing.map(t => Math.abs(t.amount))))}</strong></div><hr /><a href="#/scheduled-payments">{t("Scheduled payments")}<span>↗</span></a><a href="#/bills">{t("Bills and due dates")}<span>↗</span></a><a href="#/security">{t("Session and security")}<span>↗</span></a></State></Panel></div>
...
export function AccountsPage({ token, id, accounts, reload }: {
    ...
    return <><PageHeading title={id ? "Account details" : t("Your accounts")} description={t("A clear view of every account you hold with Nexa.")} action={!id && <button class="bank-button" onClick={() => setOpening(true)}>{t("＋ Open account")}</button>}/>{id ? <State loading={detail.loading} error={detail.error} retry={detail.reload}>{detail.data && <div class="bank-detail-grid"><AccountTile account={detail.data}/><Panel title={t("Account information")}><dl><Detail label={t("Account name")}>{detail.data.displayName}</Detail><Detail label={t("Account number")}>{detail.data.accountNumberMasked}</Detail><Detail label={t("Status")}><Status value={detail.data.status}/></Detail><Detail label={t("Ledger balance")}>{formatMoney(detail.data.ledgerBalance, detail.data.currencyCode)}</Detail><Detail label={t("Last updated")}>{detail.data.updatedAt ? formatDate(detail.data.updatedAt, true) : "Not available"}</Detail></dl><a class="bank-button" href={"#/transactions?account=" + encodeURIComponent(id)}>{t("View transactions")}</a></Panel></div>}</State> : <div class="bank-account-grid">{accounts.map(a => <AccountTile account={a} key={a.id}/>)}{!accounts.length && <State empty={t("No bank accounts yet")}/>}</div>}{opening && <OpenAccount token={token} close={() => setOpening(false)} done={() => { setOpening(false); reload(); }}/>}</>;
}
