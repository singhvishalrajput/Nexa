...
export function WorkflowCard({ workflow: w, active, busy, onAction }: {workflow: Workflow; active: boolean; busy: boolean; onAction: (command: ActionCommand) => void}) {
  ...
  const targetKind = w.operation === "PAY_CARD" || w.operation.includes("CARD") ? "cards" : w.operation === "PAY_BILL" ? "bills" : w.operation === "REPAY_LOAN" ? "loans" : w.operation === "START_TRANSFER" ? "beneficiaries" : "accounts";
  ...
  return <article class="conversation-proposal" aria-label={t("Banking proposal")} aria-busy={busy && active}>
    ...
    {active && w.status === "COLLECTING" && <div class="conversation-choices">{w.choices.map(c => <div key={c.id}><SensitiveLabel label={c.label} kind={w.field === "account" ? "accounts" : targetKind} id={c.id}/><button type="button" disabled={!enabled} aria-label={"Select " + c.label} onClick={() => action("SELECT",c.id)}>Select<span aria-hidden="true">↗</span></button></div>)}</div>}
