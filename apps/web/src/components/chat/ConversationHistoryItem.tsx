...
type Props = { conversation: Conversation; current: boolean; disabled: boolean; hasDraft: boolean; onSelect: () => void; onDelete: () => Promise<void> };

export function ConversationHistoryItem({ conversation, current, disabled, hasDraft, onSelect, onDelete }: Props) {
  ...
  return <div class={`messenger-history-item${current ? " is-current" : ""}`}>
    ...
    {confirming && <div class="messenger-history-confirm" role="group" aria-label={`Confirm deletion of ${title}`}>
