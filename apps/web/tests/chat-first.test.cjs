...
test('structured choices emit IDs rather than labels or text parsing',()=>{let sent;const card=render({workflow:{...proposal,status:'COLLECTING',choices:[{id:'12',label:'Savings'}]},onAction:c=>sent=c});nodes(card).find(n=>n.props.role==='button').props.onClick();assert.equal(sent.value,'12');assert.equal(sent.type,'SELECT');});
