...
import { CardControls, MoneyTransfer, SubscriptionManager, TransactionAnalysis, TransferDraft } from "./FinancialExperiences";
import { GoalDraft, GoalsWorkspace } from "./GoalsWorkspace";
import { BillDraft, BillsWorkspace } from "./BillsWorkspace";

type ChatWorkspaceProps = { onClose: () => void };
type Attachment = { id: string; name: string; kind: "image" | "document"; size: number };
type MessageAction = "open-dashboard" | "open-transactions" | "open-transfer" | "open-subscriptions" | "open-cards" | "open-goals" | "open-bills";
type Message = { role: "user" | "assistant"; text: string; action?: MessageAction; attachments?: Attachment[]; transfer?: TransferDraft; goal?: GoalDraft; bill?: BillDraft; insight?: "last-month-transactions" };
type ChatThread = { id: string; title: string; messages: Message[]; updatedAt: number };
type Profile = { name: string; email: string; phone: string; bankName: string; accountLabel: string; upiId: string };

const greeting: Message = { role: "assistant", text: "Good evening, Vishal. What would you like to do with your money today?" };
...
const parseGoalRequest = (value: string): GoalDraft | null => {
  if (!/\b(goals?|saving|savings|emergency\s+fund|budget(?:ing)?)\b/i.test(value)) return null;
...
const isExperienceRequest = (value: string) => isDashboardRequest(value) || isTransactionSummaryRequest(value) || isTransactionPageRequest(value) || isSubscriptionRequest(value) || isCardRequest(value) || !!parseGoalRequest(value) || !!parseBillRequest(value) || !!parseTransferRequest(value);
...
function ChatActionIcon({ action }: { action: MessageAction }) {
  return <svg viewBox="0 0 24 24" aria-hidden="true">{action === "open-dashboard" ? <><rect x="4" y="4" width="6" height="6"/><rect x="14" y="4" width="6" height="6"/><rect x="4" y="14" width="6" height="6"/><rect x="14" y="14" width="6" height="6"/></> : action === "open-transactions" ? <><path d="M5 7h14M5 12h9M5 17h11"/><path d="m16 14 3 3-3 3"/></> : action === "open-transfer" ? <><path d="M5 12h14"/><path d="m15 8 4 4-4 4"/><circle cx="7" cy="12" r="3"/></> : action === "open-subscriptions" ? <><path d="M6.5 8A7 7 0 0 1 19 10"/><path d="m19 6v4h-4"/><path d="M17.5 16A7 7 0 0 1 5 14"/><path d="m5 18v-4h4"/></> : action === "open-goals" ? <><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/><path d="M12 4V2M20 12h2"/></> : action === "open-bills" ? <><path d="M6 3h12v18l-3-2-3 2-3-2-3 2Z"/><path d="M9 8h6M9 12h6M9 16h4"/></> : <><rect x="4" y="6" width="16" height="12" rx="2"/><path d="M4 10h16M8 15h3"/></>}</svg>;
}

function loadThreads(): ChatThread[] {
  try {
    const stored = window.localStorage.getItem("nexa-chat-history");
    if (stored) return JSON.parse(stored);
  } catch (_) {}
  return [{ id: "welcome", title: "New conversation", messages: [greeting], updatedAt: Date.now() }];
}

function loadProfile(): Profile {
  const defaults = { name: "Vishal Singh", email: "vishal@example.com", phone: "+91 ••••• ••482", bankName: "Nexa Bank", accountLabel: "Primary account", upiId: "vishal@upi" };
  try {
    const stored = window.localStorage.getItem("nexa-user-profile");
    if (stored) return { ...defaults, ...JSON.parse(stored) };
  } catch (_) {}
  return defaults;
}

export function ChatWorkspace({ onClose }: ChatWorkspaceProps) {
  const [threads, setThreads] = useState<ChatThread[]>(loadThreads);
  const [currentId, setCurrentId] = useState(() => loadThreads()[0]?.id || "welcome");
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [mainView, setMainView] = useState<"chat" | "account" | "dashboard" | "transactions" | "transfer" | "subscriptions" | "cards" | "goals" | "bills">("chat");
  ...
  useEffect(() => {
    window.localStorage.setItem("nexa-chat-history", JSON.stringify(threads));
  }, [threads]);
  ...
  const submit = (value?: string) => {
    ...
    else if (transactionSummaryRequested) reply = { role: "assistant", text: "You spent ₹38,420 last month—₹4,860 less than the previous month. Home and bills was your largest category, while food spending fell by 12%.", insight: "last-month-transactions" };
  ...
  const saveProfile = (event: Event) => {
    ...
    window.localStorage.setItem("nexa-user-profile", JSON.stringify(profile));
  ...
  return <div class={`nexa-workspace ${sidebarOpen ? "is-sidebar-open" : ""}`}>
    ...
    <aside class="nexa-workspace-sidebar" id="nexa-chat-sidebar" aria-label="Chat workspace sidebar">
      ...
      <button class={`nexa-sidebar-profile ${mainView === "account" ? "is-active" : ""}`} type="button" onClick={() => { setMainView("account"); if (window.innerWidth < 900) setSidebarOpen(false); }}><span>VS</span><div><strong>{profile.name}</strong><small>Profile & bank settings</small></div><b>›</b></button>
    </aside>
    ...
    <main class="nexa-chat-main">
      <header class="nexa-chat-header">
        ...
        <span class="nexa-current-thread">{mainView === "account" ? "Account settings" : mainView === "dashboard" ? "Financial overview" : mainView === "transactions" ? "Transaction analysis" : mainView === "transfer" ? "Secure transfer" : mainView === "subscriptions" ? "Subscription manager" : mainView === "cards" ? "Card controls" : mainView === "goals" ? "Savings goals" : mainView === "bills" ? "Bills and payments" : currentThread?.title || "New conversation"}</span>
        <button class="nexa-exit-chat" type="button" onClick={onClose} aria-label="Close chat and return to site"><b aria-hidden="true">×</b></button>
      </header>
      ...
      <form class="nexa-chat-composer" onSubmit={(event) => { event.preventDefault(); submit(); }}><div class="nexa-composer-label"><label for="nexa-chat-input">{listening ? "Listening…" : "Ask Nexa"}</label></div>{attachments.length > 0 && <div class="nexa-attachment-tray">{attachments.map((file) => <div key={file.id}><b>{file.kind === "image" ? "IMG" : "DOC"}</b><span><strong>{file.name}</strong><small>{formatFileSize(file.size)}</small></span><button type="button" onClick={() => setAttachments((current) => current.filter((item) => item.id !== file.id))} aria-label={`Remove ${file.name}`}>×</button></div>)}</div>}<div class="nexa-composer-controls"><div ref={uploadWrapRef} class="nexa-upload-wrap"><button class={`nexa-upload-button ${uploadMenuOpen ? "is-open" : ""}`} type="button" onClick={() => setUploadMenuOpen((open) => !open)} aria-expanded={uploadMenuOpen} aria-haspopup="menu" aria-label="Add an attachment"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg></button>{uploadMenuOpen && <div class="nexa-upload-menu" role="menu"><span>Add to conversation</span><button type="button" role="menuitem" onClick={() => photoInputRef.current?.click()}><b>▧</b><span><strong>Photos</strong><small>JPG, PNG, WEBP and more</small></span></button><button type="button" role="menuitem" onClick={() => documentInputRef.current?.click()}><b>≡</b><span><strong>Documents</strong><small>PDF, Word, sheets and text</small></span></button></div>}<input ref={photoInputRef} class="nexa-file-input" type="file" accept="image/*" multiple onChange={(event) => addAttachments(event, "image")} /><input ref={documentInputRef} class="nexa-file-input" type="file" accept=".pdf,.doc,.docx,.txt,.csv,.xls,.xlsx,.ppt,.pptx" multiple onChange={(event) => addAttachments(event, "document")} /></div><textarea ref={textareaRef} id="nexa-chat-input" rows={1} value={input} onInput={(event) => setInput((event.currentTarget as HTMLTextAreaElement).value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); submit(); } }} placeholder={listening ? "Speak now…" : "Type or speak a request…"} /><button class={`nexa-voice-button ${listening ? "is-listening" : ""}`} type="button" onClick={toggleVoice} aria-label={listening ? "Stop listening" : "Speak to Nexa"}><svg aria-hidden="true" viewBox="0 0 24 24"><rect x="8" y="3" width="8" height="12" rx="4" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="M5.5 11.5v.5a6.5 6.5 0 0 0 13 0v-.5M12 18.5V22M9 22h6" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg></button><button class="nexa-send-button" type="submit" aria-label="Send message">↑</button></div><span>{listening ? "Voice input is active" : attachments.length ? `${attachments.length} of 5 files ready` : "Nothing happens without your approval"}</span></form></> : mainView === "account" ? <section class="nexa-account-view"><div class="nexa-account-heading"><p>Personal and banking</p><h1>Account settings</h1><span>Keep your contact information and connected account labels up to date.</span></div><form class="nexa-account-form" onSubmit={saveProfile}><div class="nexa-settings-group"><div><strong>Personal information</strong><span>Used to personalize your Nexa workspace.</span></div><div class="nexa-settings-fields"><label>Full name<input value={profile.name} onInput={(event) => setProfile({ ...profile, name: (event.currentTarget as HTMLInputElement).value })} /></label><label>Email address<input type="email" value={profile.email} onInput={(event) => setProfile({ ...profile, email: (event.currentTarget as HTMLInputElement).value })} /></label><label>Phone<input value={profile.phone} onInput={(event) => setProfile({ ...profile, phone: (event.currentTarget as HTMLInputElement).value })} /></label></div></div><div class="nexa-settings-group"><div><strong>Banking details</strong><span>Sensitive account numbers stay masked.</span></div><div class="nexa-settings-fields"><label>Bank name<input value={profile.bankName} onInput={(event) => setProfile({ ...profile, bankName: (event.currentTarget as HTMLInputElement).value })} /></label><label>Account label<input value={profile.accountLabel} onInput={(event) => setProfile({ ...profile, accountLabel: (event.currentTarget as HTMLInputElement).value })} /></label><label>UPI ID<input value={profile.upiId} onInput={(event) => setProfile({ ...profile, upiId: (event.currentTarget as HTMLInputElement).value })} /></label><div class="nexa-masked-accounts"><span><b>{profile.accountLabel}</b><small>•••• 4291</small></span><span><b>Nexa debit</b><small>•••• 4832</small></span></div></div></div><div class="nexa-account-actions"><span>{profileSaved ? "Changes saved" : "Your details are stored on this device."}</span><button type="submit">{profileSaved ? "Saved" : "Save changes"}</button></div></form></section> : mainView === "dashboard" ? <FinancialDashboard userName={profile.name} onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} /> : mainView === "transactions" ? <TransactionAnalysis onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} /> : mainView === "transfer" ? <MoneyTransfer draft={activeTransfer} onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} /> : mainView === "subscriptions" ? <SubscriptionManager onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} /> : mainView === "goals" ? <GoalsWorkspace draft={activeGoalDraft} onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} /> : mainView === "bills" ? <BillsWorkspace draft={activeBillDraft} onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} /> : <CardControls onBack={() => setMainView("chat")} onAsk={(prompt) => { setMainView("chat"); submit(prompt); }} />}
    </main>
